...
namespace Store_System.Services.ReportsService
{
    public class SaleReportService
    {
        ...
        public List<BindingOrdersAndItems> CustomBill(int id)
        {
            ...
            .Where(x => x.Order.ID==id && x.Order.IsSale == false)
        ...
        .Join(_context.Order,
              ...
              op => op.ID,
              (o, op) => new { OrderItem = o, Order = op })
        .Where(x => x.Order.OrderDate >= PreviousYear && x.Order.OrderDate <= CurrentDate && x.Order.IsSale == false)
        .Sum(x => x.OrderItem.TotalPrice);
            return query;
